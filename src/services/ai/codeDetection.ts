...
export class CodeDetectionService {
   ...
   */
  static parseMarkdownCode(content: string): { language: string; code: string; isMarkdown: boolean } {
    // Nettoyer le contenu
  ...
  static detectCodeLanguage(code: string): string {
    ...
    // Python
    if (codeLines.includes('import ') || codeLines.includes('from ') || 
        codeLines.includes('def ') || codeLines.includes('print(') ||
        codeLines.includes('if __name__') || codeLines.includes('elif ')) {
      return 'python';
    }
    ...
    // HTML
    if (codeLines.includes('<html') || codeLines.includes('<!doctype') ||
        codeLines.includes('<div') || codeLines.includes('<body')) {
      return 'html';
    }
    
    // CSS
    if (codeLines.includes('{') && codeLines.includes('}') && 
        (codeLines.includes(':') || codeLines.includes('color') || 
         codeLines.includes('margin') || codeLines.includes('padding'))) {
      return 'css';
    }
    
    // SQL
    if (codeLines.includes('select ') || codeLines.includes('insert ') ||
        codeLines.includes('update ') || codeLines.includes('delete ') ||
        codeLines.includes('create table') || codeLines.includes('from ')) {
      return 'sql';
    }
    
    // JSON
    if ((codeLines.startsWith('{') && codeLines.endsWith('}')) ||
    ...
    // PHP
    if (codeLines.includes('<?php') || codeLines.includes('$') && 
        (codeLines.includes('echo ') || codeLines.includes('function'))) {
      return 'php';
    }
    
    // Java
    if (codeLines.includes('public class') || codeLines.includes('import java') ||
        codeLines.includes('system.out.println') || codeLines.includes('public static void main')) {
      return 'java';
    }
    
    // C/C++
    if (codeLines.includes('#include') || codeLines.includes('int main') ||
        codeLines.includes('printf(') || codeLines.includes('iostream')) {
      return 'cpp';
    }
    
    // Bash/Shell
    if (codeLines.includes('#!/bin/') || codeLines.includes('echo ') ||
        codeLines.includes('cd ') || codeLines.includes('ls ')) {
      return 'bash';
    }
