...
export const AVAILABLE_TOPICS = [
  ...
  "linguistique",
  "anthropologie",
  "ecologie_environnement",
  "informatique",
  "droit",
  "musique",
  "medecine_histoire",
  "personnages_contemporains",
  "innovateurs_technologie",
  "leaders_mondials_moderne",
] as const;
...
/** AI response structure for topic analysis */
interface AITopicResponse {
  topics: Array<{ name: string; confidence: number }>;
  strategy: "topic_based" | "semantic_search" | "hybrid";
  reasoning: string;
}

/** Type guard to validate AITopicResponse structure */
function isValidAITopicResponse(value: unknown): value is AITopicResponse {
...
export class DocumentSearchService {
  ...
  private normalizeAITopics(aiTopics: string[]): string[] {
    ...
    for (const topic of aiTopics) {
      ...
      if ((AVAILABLE_TOPICS as readonly string[]).includes(normalized)) {
        ...
        const matchingTopic = AVAILABLE_TOPICS.find(
          (availableTopic) =>
            topic.toLowerCase().includes(availableTopic) ||
            availableTopic.includes(topic.toLowerCase()),
        );
...
Format exact attendu:
{
  ...
  "reasoning": "Explication courte"
      ...
      // response is now typed as AITopicResponse
