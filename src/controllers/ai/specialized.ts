...
const sanitizeUserContent = (content: string): string => {
  return content
    ...
    .replace(/(?:ignore|disregard|forget)[\s\w]*(?:previous|above|system|instructions|prompt)/gi, '[contenu filtré]')
    // Enlever les tentatives d'injection de rôle
    .replace(/(?:you are|act as|behave like|pretend to be)[\s\w]*(?:assistant|ai|system|admin)/gi, '[contenu filtré]')
    // Enlever les demandes de révélation de prompt
    .replace(/(?:show|tell|reveal|display)[\s\w]*(?:system|prompt|instructions)/gi, '[contenu filtré]')
  ...
  try {
    ...
    res.json({
      ...
      result: {
        ...
        ...(validatedData.type === 'code' && result.detectedLanguage && {
  ...
  try {
    ...
    res.json({
      ...
      result: {
        ...
        compressionRatio: (result.content.length / validatedData.content.length * 100).toFixed(1) + '%'
      }
    });
