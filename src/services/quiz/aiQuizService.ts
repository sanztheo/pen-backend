...
// Import du nouveau système Assistant via le point d'entrée centralisé
import {
  OpenAIAssistantService,
  ParallelAssistantService,
...
export class AIQuizService {
  ...
  static async generateQuiz(
    ...
    try {
      ...
      const shouldUseParallel =
        ...
        request.preset &&
        ["BREVET", "BAC", "PARTIELS"].includes(request.preset); // Preset éducatif
      ...
      if (shouldUseParallel) {
        ...
        if (processId && progressService.hasActiveConnection(processId)) {
          progressService.sendProgress(processId, {
            ...
            message:
              "Génération parallèle activée - Configuration des assistants...",
          });
        }
        ...
        console.log(
          `⚡ GÉNÉRATION PARALLÈLE ACTIVÉE: ${request.questionCount} questions pour "${subjectName}"`,
        );
        ...
        const includeDocsBySubject =
          shouldIncludeDocumentsForSubject(subjectName);
        const includeDocs =
          typeof includeDocsExplicit === "boolean"
            ? includeDocsExplicit
            : includeDocsBySubject;
        ...
        const includeGraphics =
          (request as any).graphicConfig?.enableGraphics === true;
        ...
        const subjectConfig = {
          ...
          difficulty: this.getDifficultyFromLevel(request.schoolLevel) as
            | "facile"
            | "moyen"
            | "difficile",
          ...
          documentTopics: includeDocs
            ? this.getDocumentTopicsForSubject(subjectName)
            : [],
        };
        ...
        const parallelResult =
          await parallelService.generateSingleSubjectParallel(subjectConfig);

        if (parallelResult.success && parallelResult.quiz) {
          console.log(
            `✅ Quiz parallèle généré: ${parallelResult.parallelInfo?.total_questions} questions par ${parallelResult.parallelInfo?.assistant1_questions}+${parallelResult.parallelInfo?.assistant2_questions} assistants`,
          );
          ...
          console.warn(
            `⚠️ Génération parallèle échouée: ${parallelResult.error}, fallback vers assistant unique`,
          );
      ...
      console.log(
        "🎯 Sélection du mode de génération basé sur les capacités détectées...",
      );
      ...
      if (capabilities.needsDocuments && capabilities.needsGraphics) {
        ...
        if (capabilities.useFileUpload) {
          ...
          if (documents.length === 0) {
            console.warn(
              "⚠️ Aucun document trouvé, fallback vers mode standard",
            );
          ...
          console.log(
            `📚 Trouvé ${documents.length} documents pour File Upload`,
          );
        ...
        console.log(
          "⚠️ ATTENTION: Génération en mode standard - aucun document/graphique détecté",
        );
      ...
      console.error(
        "❌ Erreur Assistant, fallback vers ancien système:",
        error,
      );
  ...
  static async generateQuizFromWorkspace(
    ...
    return QuizGenerator.generateQuizFromWorkspace(
      request,
      workspaceContent,
      ragContext,
    );
  ...
  static async correctQuiz(
    ...
    try {
      ...
      if (
        ...
        if (processId && progressService.hasActiveConnection(processId)) {
          progressService.sendProgress(processId, {
            ...
            message:
              "Correction basée uniquement sur le contenu de vos cours...",
          });
        }
      ...
      if (
        correctionCapabilities.hasGraphics &&
        correctionCapabilities.hasDocuments
      ) {
  ...
  static async analyzeWorkspaceContent(
    ...
    return WorkspaceAnalyzer.analyzeWorkspaceContent(
      workspaceContent,
      workspaceName,
      schoolLevel,
    );
  ...
  private static detectQuizCapabilities(request: QuizGenerationRequest): {
    ...
    console.log(
      "📝 detectQuizCapabilities - Sujet détecté:",
      subjectName,
      "(lowercase:",
      subject + ")",
    );
    ...
    if (request.documentConfig && request.documentConfig.enableDocuments) {
      console.log(
        "🔍 Configuration documentaire détectée pour séquence:",
        request.documentConfig,
      );
      console.log(
        "📚 Documents activés via configuration explicite pour:",
        subjectName,
      );
      needsDocuments = true;
      ...
      console.log("✅ Résultat configuration explicite:", {
        needsDocuments,
        documentTopics,
        useFileUpload,
      });
      ...
      console.log(
        "⚠️ Pas de documentConfig ou enableDocuments=false, passage à la détection automatique",
      );
      console.log(
        "🔍 request.documentConfig:",
        request.documentConfig || "undefined",
      );
      ...
      console.log("🔧 Détection automatique par mots-clés:", {
        needsDocuments,
        subject,
      });
    }
    ...
    if (needsDocuments && documentTopics.length === 0) {
      if (subject.includes("histoire") || subject.includes("révolution")) {
        ...
        subject.includes("littérature") ||
        subject.includes("français")
      ) {
    ...
    if (!useFileUpload && needsDocuments && documentTopics.length > 0) {
      useFileUpload = true;
      console.log(
        "📤 File Upload activé car documents nécessaires et topics disponibles",
      );
    }
    ...
    const result = {
      ...
      needsDocuments,
      useFileUpload,
      graphicType: (subject.includes("3d") || subject.includes("espace")
        ? "3d"
        : "2d") as "2d" | "3d",
      library: (subject.includes("plotly") || subject.includes("3d")
        ? "plotly"
        : "apexcharts") as "apexcharts" | "plotly",
      documentTopics,
      subjectName,
    };
   ...
   */
  private static getDifficultyFromLevel(
    schoolLevel: SchoolLevel,
  ...
  private static transformAssistantResult(
    ...
    const aiCorrection = {
      globalFeedback: this.generateGlobalFeedback(
        assistantResult,
        percentage,
        questionResults,
      ),
  ...
  private static extractStrengths(
    ...
    if (correctAnswers.length > 0) {
      strengths.push(
        `Bonnes réponses sur ${correctAnswers.length} question(s)`,
      );
    ...
    return strengths.length > 0
      ? strengths
      : ["Participation au quiz complétée"];
  }
  ...
  private static extractWeaknesses(
    ...
    if (
      assistantResult.weaknesses &&
      Array.isArray(assistantResult.weaknesses)
    ) {
  ...
  private static generateGlobalFeedback(
    ...
    if (percentage >= 80) {
      ...
      feedback +=
        "Travail correct. Une révision des concepts serait bénéfique.";
    } else {
      feedback +=
        "Des efforts supplémentaires sont nécessaires. Reprenez les bases du sujet.";
    }
  ...
  > {
    try {
      ...
      console.log(
        `✅ Documents sélectionnés: ${filteredChunks.length} sur ${searchResult.chunks.length} trouvés`,
      );
   ...
   */
  private static truncateOnSentenceEnd(
    text: string,
    maxLength: number,
  ): string {
