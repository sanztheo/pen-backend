...
export async function buildPagesContext(
  ...
  for (const p of pages) {
    const text = Array.isArray(p.blockNoteContent) ? extractTextFromBlockNote(p.blockNoteContent as any[]) : '';
    const slice = typeof maxCharsPerPage === 'number'
      ? maxCharsPerPage
      : (pageIds && pageIds.length ? 8000 : 1500);
...
export function extractParagraphsFromBlockNote(blocks: any[]): string[] {
...
export async function buildPagesContextChunked(
  ...
  for (const p of pages) {
    const blocks = Array.isArray(p.blockNoteContent) ? (p.blockNoteContent as any[]) : [];
  ...
  const meaningfulTokens = query
    ...
    .filter(Boolean)
    .filter((w) => w.length >= 3 && !['bonjour','salut','coucou','hello','hey','yo','merci','svp','stp','ok','okay','yes','no'].includes(w));
