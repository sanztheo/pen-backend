...
export const requireAICredits = (config: AICreditsConfig = {}) => {
  return async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (!userId) {
        return res.status(401).json({ success: false, error: 'Utilisateur non authentifié', code: 'UNAUTHORIZED' });
      }

      // SÉCURITÉ: Vérifier que l'utilisateur existe en base de données
      const userExists = await prisma.user.findUnique({ where: { id: userId }, select: { id: true } });
      if (!userExists) {
              secureLog('error: ❌ [AI-CREDITS] Tentative d\'utilisation pour un utilisateur inexistant', { userId });
      ...
      if (!canUse) {
        secureLog('warn: 🚨 [AI-CREDITS] Tentative usage IA sans crédits', { userId, path: req.path, action });
        return res.status(403).json({ success: false, error: 'Limite de crédits IA atteinte', code: 'CREDITS_EXHAUSTED', limitReached: true });
      }

      // 2. Déduire les crédits
      const deductionResult = await AICreditsService.deductCredits(userId, cost, action);
      if (!deductionResult.success) {
        secureLog('warn: 🚨 [AI-CREDITS] Échec déduction crédits', { userId, path: req.path, action, cost, remainingCredits: deductionResult.remainingCredits });
        return res.status(403).json({ success: false, error: deductionResult.message, code: 'CREDITS_DEDUCTION_FAILED', remainingCredits: deductionResult.remainingCredits, limitReached: deductionResult.limitReached });
      }

      // 3. Ajouter infos crédits à la requête
      (req as any).aiCredits = { cost, remainingCredits: deductionResult.remainingCredits, action };

      secureLog('debug: ✅ [AI-CREDITS] Crédits déduits', { userId, path: req.path, action, cost, remainingCredits: deductionResult.remainingCredits });

      next();
    } catch (error) {
            secureLog('error: ❌ [AI-CREDITS] Erreur middleware crédits IA', error);
      return res.status(500).json({ success: false, error: 'Erreur interne lors de la vérification des crédits', code: 'CREDITS_CHECK_ERROR' });
    }
  };
};
...
export default requireAICredits;
