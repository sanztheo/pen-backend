...
export class DocumentBasedQuizGenerator {
  ...
  private deduplicateDocumentChunks(chunks: DocumentChunk[]): DocumentChunk[] {
    ...
    console.log(`🗡️ Déduplication: ${chunks.length} chunks → ${docMap.size} documents uniques`);
   ...
   */
  private buildSearchStrategies(subjectName: string, topics: string[]): Array<{
    name: string;
    query: string;
    threshold: number;
    topics: string[];
  ...
  async generateDocumentBasedQuiz(
    ...
    console.log(`📊 Documents trouvés: ${documents.chunks.length} en ${searchTime}ms`);
    ...
    const documentMetadata: DocumentBasedQuizMetadata = {
      ...
      searchStrategy: (documents.search_strategy as 'topic_based' | 'semantic_search' | 'hybrid') || 'semantic_search',
  ...
  private async searchRelevantDocuments(
    subjectName: string, 
    config: { topics?: string[]; maxDocuments: number; minDocumentLength: number }
  ) {
    // Détermine les topics à utiliser
    const subjectKey = subjectName.toLowerCase();
    const mappedTopics = SUBJECT_TOPIC_MAPPING[subjectKey as keyof typeof SUBJECT_TOPIC_MAPPING] || [];
    
    const searchTopics = config.topics || [...mappedTopics];
    
    console.log(`🔍 Recherche documentaire pour "${subjectName}" avec topics:`, searchTopics);
    ...
    for (const strategy of searchStrategies) {
      console.log(`🎯 Tentative ${attemptNumber}/${searchStrategies.length}: "${strategy.query}" (seuil: ${strategy.threshold})`);
      ...
      console.log(`📊 Résultats tentative ${attemptNumber}: ${filteredChunks.length} documents valides`);
      ...
      if (filteredChunks.length > 0) {
        ...
        console.log(`✅ Documents sélectionnés: ${selectedChunks.length} uniques sur ${filteredChunks.length} chunks (stratégie: ${strategy.name})`);
    ...
    if (!finalResult) {
      console.log(`⚠️ Aucun document pertinent trouvé après ${searchStrategies.length} tentatives`);
      finalResult = {
        chunks: [],
  ...
  private async generateQuizWithDocuments(
    ...
    console.log(`📝 Génération: ${documentQuestions} questions docs + ${knowledgeQuestions} questions connaissances`);
    ...
    console.log(`🤖 Génération du quiz avec ${documents.length} documents intégrés`);
...
Format de réponse attendu (JSON uniquement) :
{
   ...
   */
  private parseQuizResponse(content: string): { questions: Question[]; metadata: any } {
    try {
      ...
      const transformedQuestions = parsed.questions.map((q: any) => this.transformQuestionFormat(q));
   ...
   */
  private transformQuestionFormat(aiQuestion: any): Question {
    const baseQuestion = {
    ...
    switch (aiQuestion.type) {
      case 'MULTIPLE_CHOICE':
        ...
        const options = (aiQuestion.choices || []).map((choice: string, index: number) => ({
          id: String(index),
          text: choice,
          isCorrect: index === 0 // Par défaut, première option correcte (à ajuster selon correctAnswer)
        }));

        // Ajuster la bonne réponse si correctAnswer est fourni
        if (aiQuestion.correctAnswer) {
          const correctIndex = (aiQuestion.choices || []).findIndex((choice: string) => 
            choice === aiQuestion.correctAnswer || 
            choice.startsWith(aiQuestion.correctAnswer)
          );
          if (correctIndex >= 0) {
      ...
      case 'OPEN_QUESTION':
        return {
          ...baseQuestion,
          type: 'OPEN_QUESTION',
          expectedAnswer: aiQuestion.correctAnswer || aiQuestion.explanation,
  ...
  private markDocumentQuestions(questions: Question[], documentQuestionCount: number): Question[] {
    return questions.map((question, index) => ({
      ...
      documentReference: index < documentQuestionCount ? 
