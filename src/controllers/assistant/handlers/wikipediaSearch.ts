...
export const wikipediaSearch = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    console.log(`🔍 [WikipediaSearch] Recherche: "${searchQuery}" (lang: ${lang})`);
    ...
    if (!searchData.query?.search) {
      console.log(`⚠️ [WikipediaSearch] Aucun résultat trouvé pour: "${searchQuery}"`);
      res.json({ articles: [], totalFound: 0 });
      return;
    }

    const searchResults = searchData.query.search;
    console.log(`📊 [WikipediaSearch] ${searchResults.length} résultats trouvés`);

    // Étape 2: Récupérer les extraits des articles les plus pertinents
    const pageIds = searchResults.slice(0, Math.min(searchResults.length, 10)).map((r: any) => r.pageid);
    ...
    for (const searchResult of searchResults.slice(0, 10)) {
      ...
      if (pageData && !pageData.missing) {
        const article: WikipediaArticle = {
          ...
          url: pageData.fullurl || `https://${lang}.wikipedia.org/wiki/${encodeURIComponent(pageData.title || searchResult.title)}`
        };
        ...
        console.log(`✅ [WikipediaSearch] Article ajouté: "${article.title}" (${article.extract.length} chars)`);
      }
    }
    ...
    console.log(`🎯 [WikipediaSearch] Retour de ${articles.length} articles pour "${searchQuery}"`);
    res.json(result);
...
export const wikipediaGetEnrichedArticles = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    console.log(`🔍 [WikipediaEnriched] Mode: ${mode}, Pages: ${pageIds.length}, Query: "${query}"`);
    ...
    for (const pageId of pageIds) {
      ...
      if (!contentResponse.ok) {
        console.error(`❌ Erreur récupération article ${pageId}: ${contentResponse.status}`);
        continue;
      }
      ...
      const categories = pageData.categories?.map((cat: any) => 
        cat.title.replace('Catégorie:', '').replace('Category:', '')
      ) || [];

      console.log(`📂 [${pageData.title}] ${categories.length} catégories trouvées:`, categories.slice(0, 5));
      ...
      if (mode === 'search' || (mode === 'create' && req.body.reflection === 'profond')) {
        ...
        console.log(`🎯 [${pageData.title}] ${relevantCategories.length} catégories pertinentes sélectionnées`);
        
        // Récupérer le contenu des catégories pertinentes
        if (relevantCategories.length > 0) {
          categoryContent = await getCategoryContent(relevantCategories, lang, Math.floor(charLimit * 0.2));
          console.log(`📖 [${pageData.title}] Contenu des catégories: ${categoryContent.length} chars`);
        } else {
          console.log(`⚠️ [${pageData.title}] Aucune catégorie pertinente, utilisation de toutes les catégories`);
      ...
      const article: WikipediaArticle = {
        ...
        url: pageData.fullurl || `https://${lang}.wikipedia.org/wiki/${encodeURIComponent(pageData.title)}`,
        categories: relevantCategories.length > 0 ? relevantCategories : categories.slice(0, 10),
        fullContent: categoryContent ? `${pageData.extract}\n\n=== Contenu des catégories pertinentes ===\n${categoryContent}` : pageData.extract
      };

      articles.push(article);
      console.log(`✅ [WikipediaEnriched] Article traité: "${article.title}" (${article.fullContent?.length || 0} chars)`);
    }
...
async function analyzeRelevantCategories(categories: string[], query: string): Promise<string[]> {
  ...
  categories.forEach(category => {
    ...
    const importantKeywords = [
      'histoire', 'politique', 'science', 'physique', 'mathématiques', 'géographie', 
      'économie', 'société', 'culture', 'art', 'littérature', 'philosophie',
      'guerre', 'conflit', 'militaire', 'stratégie', 'bataille', 'armée',
      'théorie', 'principe', 'loi', 'concept', 'définition'
    ];
    ...
    const excludePatterns = [
      'maintenance', 'ébauche', 'homonymie', 'redirection', 'page', 'projet',
      'portail', 'catégorie', 'modèle', 'utilisateur'
    ];
 ...
 */
async function getCategoryContent(categories: string[], lang: string, maxChars: number): Promise<string> {
  ...
  for (const category of categories) {
    ...
    try {
      ...
      if (members.length > 0) {
        const memberTitles = members.slice(0, 3).map((m: any) => `• ${m.title}`).join('\n');
...
export const wikipediaGetArticle = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    console.log(`📄 [WikipediaGetArticle] Récupération: pageid=${pageid}, title=${title} (lang: ${lang})`);
    ...
    console.log(`✅ [WikipediaGetArticle] Article récupéré: "${article.title}" (${article.extract.length} chars)`);
    res.json({ article });
