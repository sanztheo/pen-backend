import { QuestionType, SchoolLevel, CollegeGrade, QuizPreset, ExamSubject, QuizGenerationRequest } from '../types.js';
...
export class PromptUtils {
  ...
  static getGenerationPrompt(request: QuizGenerationRequest): string {
    ...
    const specialtiesDebug = request.specialties || request.lyceeSpecialties || request.selectedSpecialties;
    console.log(`📚 [CUSTOM] Génération prompt personnalisé: ${request.schoolLevel} - Spécialités: ${specialtiesDebug?.join(', ') || 'N/A'} - Types: ${request.questionTypes?.join(', ') || 'N/A'}`);
  ...
  static getCustomPrompt(request: QuizGenerationRequest): string {
    ...
    if (specialtiesList && specialtiesList.length > 0) {
      const specialtiesStr = specialtiesList.map(s => {
      ...
      customizations.push(`- MATIÈRES SPÉCIFIQUES : Concentre-toi UNIQUEMENT sur ${specialtiesStr}`);
    }
    
    // Filière études supérieures
    if (request.higherEdField && request.schoolLevel === 'ETUDES_SUPERIEURES') {
      customizations.push(`- FILIÈRE D'ÉTUDES : Toutes les questions doivent être spécifiques à ${request.higherEdField}`);
    }
    ...
    if (request.questionTypes && request.questionTypes.length > 0) {
      ...
      if (typeCount === 1) {
        ...
        customizations.push(`- TYPES DE QUESTIONS : Génère ${questionCount} questions EXCLUSIVEMENT de type ${typeName}`);
      } else {
        ...
        customizations.push(`- RÉPARTITION OBLIGATOIRE DES TYPES : Sur ${questionCount} questions totales, génère EXACTEMENT :`);
        distributionDetails.forEach(detail => {
          customizations.push(`  • ${detail}`);
        });
        customizations.push(`-  CRITIQUE : Cette répartition est STRICTEMENT OBLIGATOIRE. Ne génère PAS uniquement des QCM !`);
      }
    }
    
    // Difficulté
    if ((request as any).difficulty && (request as any).difficulty !== 'adaptatif') {
      customizations.push(`- NIVEAU DE DIFFICULTÉ : ${(request as any).difficulty.toUpperCase()}`);
    }
...
- CONTRÔLE FINAL : Vérifie que tu as bien respecté la répartition des types avant de finaliser`;
      ...
      if (request.questionTypes && request.questionTypes.length > 0) {
        const typeInstructions = request.questionTypes.map(type => this.getQuestionTypePrompt(type)).join('\n');
...
- Vérification finale obligatoire : Compte le nombre de questions de chaque type avant de finaliser`;
   ...
   */
  static getPresetPrompt(preset: QuizPreset, subject?: ExamSubject, request?: QuizGenerationRequest): string {
    switch (preset) {
      case QuizPreset.BREVET:
        ...
        if (request?.specificSubject) {
          console.log(`📝 [BREVET] Génération prompt pour specificSubject: ${request.specificSubject}`);
      ...
      case QuizPreset.BAC:
        ...
        // Vérifier s'il y a un specificSubject dans la requête
        if (request?.specificSubject) {
          console.log(`🎓 [BAC] Génération prompt pour specificSubject: ${request.specificSubject}`);
      ...
      case QuizPreset.PARTIELS:
        if (subject && request?.higherEdField) {
          ...
          console.log(`📚 [PARTIELS] Génération prompt pour: ${request.higherEdField} - ${subjectName}`);
        ...
        if (request?.specificSubject && request?.higherEdField) {
          ...
          console.log(`📚 [PARTIELS] Génération prompt pour specificSubject: ${request.higherEdField} - ${subjectName}`);
          return getPartielsPrompt(request.higherEdField, subjectName);
        }
        console.log(`📚 [PARTIELS] Génération prompt par défaut: ${request?.higherEdField || 'Général'}`);
        return getPartielsPrompt(request?.higherEdField || 'Général', 'Matière générale');

      default:
        console.log(`⚠️ [PRESET] Preset non reconnu: ${preset}, fallback vers prompt générique`);
        // Fallback vers le prompt générique
        return this.getGenerationPromptByLevel(request?.schoolLevel || SchoolLevel.COLLEGE, request?.collegeGrade);
    }
  }
   ...
   */
  private static getPartielsSubjectName(request: QuizGenerationRequest, subject: ExamSubject): string {
    // Utiliser le nom stocké dans subjectResults quand disponible
    if (request.sequentialConfig && 
        typeof request.sequentialConfig.currentSubjectIndex === 'number') {
      
      const currentSubjectResult = request.sequentialConfig.subjectResults?.[request.sequentialConfig.currentSubjectIndex];
