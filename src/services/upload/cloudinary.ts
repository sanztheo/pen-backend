...
function validateFile(buffer: Buffer, mimetype: string): UploadError | null {
  ...
  if (!isAllowedMimeType(mimetype)) {
    return {
      ...
      details: { receivedType: mimetype, allowedTypes: UPLOAD_CONFIG.ALLOWED_IMAGE_TYPES }
    };
  }
...
async function compressImage(buffer: Buffer, mimetype: string): Promise<Buffer> {
  try {
    ...
    throw new Error(`Échec compression: ${error.message}`);
  }
}
...
export async function deleteFromCloudinary(publicId: string): Promise<void> {
  ...
  try {
    ...
    if (result.result === 'ok') {
      console.log('✅ [Cloudinary Service] Image SUPPRIMÉE avec succès de Cloudinary');
    } else if (result.result === 'not found') {
      console.warn('⚠️ [Cloudinary Service] Image déjà supprimée ou introuvable sur Cloudinary');
    } else {
      console.warn('⚠️ [Cloudinary Service] Statut suppression inattendu:', result.result);
    }
  } catch (error: any) {
