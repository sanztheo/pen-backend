...
export interface AdvancedQuizStats {
  ...
  targetGrade: number | null; // Note cible de l'utilisateur (0-20)
}
...
export class StatsService {
  ...
  static async getAdvancedUserStats(
    ...
    // Récupérer les préférences utilisateur pour obtenir targetGrade
    const userPreferences = await prisma.userQuizPreferences.findUnique({
      where: { userId },
    ...
    if (completedQuizzes.length === 0) {
      return {
        ...
        currentStreak: 0,
        longestStreak: 0,
        targetGrade: userPreferences?.targetGrade ?? null,
      };
    }
