...
const PresetSequenceSchema = z.object({
  preset: z.enum(['BREVET', 'BAC', 'PARTIELS'], {
    errorMap: () => ({ message: 'Le preset doit être BREVET, BAC ou PARTIELS' })
  }),
...
export function requireCustomQuizLimits(autoDeduct: boolean = true) {
  return async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      SecureLogger.debug(`🎯 [QUIZ-MIDDLEWARE] Vérification limites quiz personnalisés`, { userId, autoDeduct });
      ...
      if (!canCreate.success) {
        SecureLogger.warn(`❌ [QUIZ-MIDDLEWARE] Limite quiz personnalisés atteinte`, { userId, remainingQuizzes: canCreate.remainingQuizzes });
      ...
      if (autoDeduct) {
        ...
        if (!deductResult.success) {
          SecureLogger.warn(`❌ [QUIZ-MIDDLEWARE] Échec déduction quiz personnalisé`, { userId, message: deductResult.message });
        ...
        SecureLogger.debug(`✅ [QUIZ-MIDDLEWARE] Quiz personnalisé déduit`, { userId, remainingQuizzes: deductResult.remainingQuizzes });
...
export function requirePresetSequenceLimits(autoStart: boolean = false) {
  return async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      SecureLogger.debug(`🎯 [QUIZ-MIDDLEWARE] Vérification limites séquences preset`, { userId, preset, autoStart });
      ...
      if (autoStart) {
        ...
        const startResult = await QuizLimitsService.startPresetSequence(userId, preset, sequenceData);
        
        if (!startResult.success) {
          SecureLogger.warn(`❌ [QUIZ-MIDDLEWARE] Échec démarrage séquence preset`, { userId, preset, message: startResult.message });
          return res.status(403).json({
            success: false,
        ...
        SecureLogger.debug(`✅ [QUIZ-MIDDLEWARE] Séquence preset démarrée`, { userId, preset, sequenceId: startResult.sequenceId });
  ...
  return (req: AuthRequest, res: Response, next: NextFunction) => {
    ...
    res.json = function(body: any) {
      ...
      if (res.statusCode >= 400 && req.quizLimits?.deducted && req.user?.id) {
        ...
        SecureLogger.debug(`🔄 [QUIZ-MIDDLEWARE] Remboursement automatique détecté`, { userId, limitType, statusCode: res.statusCode });
        
        // Remboursement asynchrone (ne pas bloquer la réponse)
        QuizLimitsService.refundQuiz(userId, limitType === 'custom' ? 'custom' : 'preset')
          .then((refundResult) => {
            if (refundResult.success) {
              SecureLogger.debug(`✅ [QUIZ-MIDDLEWARE] Remboursement automatique réussi`, { userId, limitType });
            } else {
              SecureLogger.error(`❌ [QUIZ-MIDDLEWARE] Échec remboursement automatique`, { userId, limitType, error: refundResult.message });
            }
          })
          .catch((error) => {
