...
export class SourceSelectionService {
  static async selectSources(context: SourceSelectionContext): Promise<SourceSelectionResult> {
    ...
    DebugLogger.rag(
      `Sélection sources - query: "${query}", ragSources: ${ragSources.length}, scope: ${sourcesScope}`,
    );
   ...
   */
  private static async selectAllWorkspaceSources(
    query: string,
    workspaceId: string,
  ): Promise<string[]> {
    try {
   ...
   */
  private static smartFallbackSelection(
    query: string,
