...
- FERMETURE OBLIGATOIRE: Toute formule DOIT être correctement fermée. Chaque $ doit avoir son $ fermant.
...
- Encadre TOUTE formule par $...$ UNIQUEMENT - VÉRIFIER la fermeture avant de valider.
- Ne mets JAMAIS de texte français (ou caractères accentués) ENTRE les délimiteurs $...$.
- Évite toute macro de section (\\section, \\subsection, etc.) et les environnements (equation, align, etc.). Utilise uniquement $ avec du contenu math pur.
...
- VALIDATION: Avant de valider ton contenu, compte tes délimiteurs $ pour t'assurer qu'ils sont équilibrés.
- Quand tu écris une définition avec explication: $c^2 = a^2 + b^2 - 2ab \\cos(\\gamma)$ — où $c$ est le côté opposé à $\\gamma$.
- FRACTIONS: TOUJOURS utiliser \\frac{numérateur}{dénominateur}, JAMAIS a/b en texte brut.
- Exemples fractions: $\\frac{1}{2}$, $\\frac{a+b}{c}$, $\\frac{2 \\times 2}{5 \\times 2} = \\frac{4}{10}$
`;