...
import { uploadToCloudinary, deleteFromCloudinary, UPLOAD_CONFIG } from '../services/upload/cloudinary.js';
...
router.delete(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      // Format: pennote/notes/userId_timestamp
      const userId = req.user.id;
      console.log('🔐 [Route DELETE] Vérification ownership:', {
        userId,
        publicId,
        publicIdContainsUserId: publicId.includes(userId)
      });
      ...
      console.log('✅ [Route DELETE] Validation réussie, appel service Cloudinary...');
