...
const TOOL_DEPENDENCIES = {
  ...
  select_relevant_sources: {
    requiredPrevious: ['list_available_sources', 'list_global_wikipedia_sources'],
    requiredArguments: ['question', 'availableSources'],
    argumentSources: {
      availableSources: ['list_available_sources', 'list_global_wikipedia_sources']
    }
  },
  ...
  read_rag_source: {
    ...
    argumentSources: {
      sourceId: ['list_available_sources', 'list_global_wikipedia_sources', 'select_relevant_sources']
    }
  },
  ...
  check_sources_rag_status: {
    ...
    argumentSources: {
      sourceIds: ['list_available_sources', 'list_global_wikipedia_sources', 'select_relevant_sources']
    }
  }
};
...
export class ToolDependenciesValidator {
  ...
  static validateDependencies(
    ...
    const dependencies = TOOL_DEPENDENCIES[toolName as keyof typeof TOOL_DEPENDENCIES];
    if (!dependencies) {
      return {
        isValid: true,
    ...
    if (dependencies.requiredPrevious) {
      const hasRequiredPrevious = dependencies.requiredPrevious.some(reqTool =>
    ...
    if (dependencies.requiredArguments) {
      for (const argName of dependencies.requiredArguments) {
        if (!toolArguments || toolArguments[argName] === undefined || toolArguments[argName] === null) {
    ...
    if (dependencies.argumentSources && toolArguments) {
      for (const [argName, sourcesTools] of Object.entries(dependencies.argumentSources)) {
        const argValue = toolArguments[argName];

        // Cas spécial : select_relevant_sources
        if (toolName === 'select_relevant_sources' && argName === 'availableSources') {
        ...
        if (toolName === 'check_sources_rag_status' && argName === 'sourceIds') {
    ...
    if (missingDependencies.length > 0) {
      console.warn(`❌ [DEPENDENCIES] ${toolName} a des dépendances manquantes:`, missingDependencies);
  ...
  private static validateAvailableSources(
    ...
    if (!Array.isArray(availableSources) || availableSources.length === 0) {
      ...
      if (context.extractedSources && context.extractedSources.length > 0) {
        console.log(`🔧 [DEPENDENCIES] availableSources vide, correction automatique avec extractedSources`);

        return {
          isValid: true,
          reasoning: 'availableSources corrigé automatiquement depuis extractedSources',
          suggestedFix: {
      ...
      return {
        isValid: false,
        reasoning: 'select_relevant_sources requiert availableSources (array non vide). Appeler list_available_sources ou list_global_wikipedia_sources d\'abord.',
        missingDependencies: ['list_available_sources ou list_global_wikipedia_sources'],
        shouldBlock: true
      };
    }

    // RÈGLE : Chaque source doit avoir {id, title, sourceType}
    const invalidSources = availableSources.filter((src: any) =>
      !src.id || !src.title || !src.sourceType
    );
  ...
  private static validateSourceId(
    ...
    if (typeof sourceId !== 'string' || sourceId.trim().length === 0) {
      return {
        isValid: false,
        reasoning: 'read_rag_source requiert sourceId (string non vide). Ne JAMAIS appeler avec un ID vide.',
        shouldBlock: true
      };
    }
    ...
    if (context.extractedSources && context.extractedSources.length > 0) {
      const isValidSource = context.extractedSources.some(src => src.id === sourceId);

      if (!isValidSource) {
        console.warn(`⚠️ [DEPENDENCIES] sourceId "${sourceId}" non trouvé dans extractedSources`);
        console.warn(`   Sources disponibles: ${context.extractedSources.map(s => s.id).join(', ')}`);
  ...
  private static validateSourceIds(
    ...
    if (!Array.isArray(sourceIds) || sourceIds.length === 0) {
      return {
        isValid: false,
        reasoning: 'check_sources_rag_status requiert sourceIds (array non vide)',
        shouldBlock: true
      };
    }

    // RÈGLE : Tous les IDs doivent être des strings non vides
    const invalidIds = sourceIds.filter(id => typeof id !== 'string' || id.trim().length === 0);
    ...
    if (context.extractedSources && context.extractedSources.length > 0) {
      ...
      if (unknownIds.length > 0) {
        console.warn(`⚠️ [DEPENDENCIES] ${unknownIds.length} sourceIds inconnus:`, unknownIds);
   ...
   */
  static detectMode(query: string, isSearch: boolean): 'ask' | 'search' | 'create_rapide' | 'create_profond' {
    const queryLower = query.toLowerCase();

    // Détection de create
    const createKeywords = ['crée', 'créer', 'génère', 'générer', 'écris', 'écrire', 'rédige', 'rédiger', 'compose', 'construis', 'fais', 'faire'];
    const isCreate = createKeywords.some(kw => queryLower.includes(kw));

    if (isCreate) {
      // Détection de profond vs rapide
      const profondKeywords = ['détaillé', 'approfondi', 'complet', 'exhaustif', 'en profondeur', 'analyse'];
      const isProfond = profondKeywords.some(kw => queryLower.includes(kw)) || isSearch;
   ...
   */
  static getToolLimits(mode: 'ask' | 'search' | 'create_rapide' | 'create_profond'): {
    minTools: number;
    maxTools: number;
    recommended: number;
  } {
    switch (mode) {
      case 'ask':
        return { minTools: 0, maxTools: 3, recommended: 1 };
      case 'search':
        return { minTools: 2, maxTools: 10, recommended: 5 };
      case 'create_rapide':
        return { minTools: 0, maxTools: 3, recommended: 2 };
      case 'create_profond':
        return { minTools: 2, maxTools: 10, recommended: 6 };
    }
  }
  ...
  static validatePlan(
    ...
    if (toolSequence.length > limits.maxTools) {
      console.warn(`⚠️ [DEPENDENCIES] Plan dépasse la limite (${toolSequence.length} > ${limits.maxTools})`);
    }
    ...
    if (selectIdx !== -1) {
      const hasListingBefore = toolNames.slice(0, selectIdx).some(name =>
        name === 'list_available_sources' || name === 'list_global_wikipedia_sources'
      );

      if (!hasListingBefore) {
        return {
          isValid: false,
          reasoning: 'select_relevant_sources DOIT être après list_available_sources ou list_global_wikipedia_sources',
          shouldBlock: true
        };
      }
    }
    ...
    if (readIdx !== -1) {
      const hasSourcesBefore = toolNames.slice(0, readIdx).some(name =>
      ...
      if (!hasSourcesBefore) {
        return {
          isValid: false,
          reasoning: 'read_rag_source DOIT être après un tool de listing/sélection de sources',
          shouldBlock: true
        };
      }
    }
