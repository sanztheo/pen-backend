...
export class SubjectGenerator {
  ...
  static async generateSubjects(request: QuizGenerationRequest): Promise<QuizSubject[]> {
    ...
    console.log(`📚 Génération optimisée de ${subjectCount} sujets avec ${questionsPerSubject} questions chacun`);
...
`;
    ...
    try {
      ...
      return data.subjects.map((subjectData: any, index: number) => ({
        id: uuidv4(),
        title: subjectData.title,
        description: subjectData.description,
        questions: subjectData.questions.map((q: any) => {
          // Transformer choices en options pour les questions à choix multiples
          ...
          if (q.type === 'MULTIPLE_CHOICE' && q.choices && Array.isArray(q.choices)) {
            transformedQuestion.options = q.choices.map((choice: string, choiceIndex: number) => ({
              id: `option_${choiceIndex + 1}`,
              text: choice,
              isCorrect: false // À définir côté correction
            }));
            // Supprimer l'ancien champ choices
            delete transformedQuestion.choices;
          }
      ...
      console.warn('⚠️ Erreur génération optimisée, fallback vers méthode individuelle:', error);
  ...
  private static async generateSingleSubject(
    ...
    const questions = await this.generateQuestionsForSubject(request, subjectTheme, questionCount, documents);
...
`;
   ...
   */
  private static async generateAIDocument(theme: { title: string; category: string }): Promise<SubjectDocument[]> {
    const prompt = `
...
`;
  ...
  private static subjectNeedsDocuments(request: QuizGenerationRequest): boolean {
    ...
    return subjectName.includes('histoire') || 
  ...
  private static getSubjectName(request: QuizGenerationRequest): string {
    ...
    if (request.preset === 'PARTIELS' && request.sequentialConfig && request.higherEdField) {
      ...
      if (config.subjectResults && config.currentSubjectIndex !== undefined) {
        ...
        if (currentSubjectResult && currentSubjectResult.subjectName) {
          console.log(`🎯 Matière Partiel détectée: ${currentSubjectResult.subjectName} (matière spécifique de ${request.higherEdField})`);
    ...
    if (request.higherEdField) {
      console.log(`🎯 Matière détectée: ${request.higherEdField} (filière d'études supérieures)`);
