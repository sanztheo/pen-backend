...
const autocompleteSchema = z.object({
  ...
  blockType: z.enum(['text', 'heading2', 'heading3', 'list', 'quote', 'code']).optional(),
...
const checkAutocompletionEnabled = async (userId: string): Promise<boolean> => {
  try {
    ...
    console.error('❌ Erreur lors de la vérification des préférences utilisateur:', error);
  ...
  try {
    ...
    if (!isEnabled) {
      return res.status(403).json({ 
        ...
        message: 'L\'autocomplétion est désactivée dans vos préférences utilisateur',
    ...
    req.on('close', () => {
      if (!abortController.signal.aborted) {
        console.log('🚫 [AUTOCOMPLETE] Connexion client fermée, annulation de la requête');
...
export const handleAutocompleteWebSocket = (ws: WebSocket, req: any) => {
    ...
    try {
      ...
      try {
        ...
        console.error('❌ [WS-AUTOCOMPLETE] Message JSON invalide:', {
          ...
          error: parseError instanceof Error ? parseError.message : 'Erreur de parsing inconnue',
          userId: user.id
        });
      ...
      if (data.type === 'autocomplete') {
        ...
        if (!isEnabled) {
          console.log(`🚫 [WS-AUTOCOMPLETE] Autocomplétion désactivée pour l'utilisateur ${user.id}`);
          
          if (ws.readyState === WebSocket.OPEN) {
            ws.send(JSON.stringify({
              type: 'error',
              error: 'Autocomplétion désactivée dans vos préférences utilisateur',
              code: 'AUTOCOMPLETION_DISABLED',
              timestamp: Date.now()
            }));
          }
        ...
        cancelCurrentRequest('Nouvelle requête reçue, annulation de la précédente');
        ...
        console.log(`🚀 [WS-AUTOCOMPLETE] Démarrage autocomplétion streaming [${requestId}] pour utilisateur ${user.id}...`);
        ...
        try {
          ...
          await AIService.autocompleteStream(
            ...
            (streamResult: AutocompleteStreamResult) => {
              ...
              if (currentAbortController?.signal.aborted || lastRequestId !== requestId) {
                console.log(`🚫 [WS-AUTOCOMPLETE] Chunk ignoré - requête [${requestId}] annulée`);
                return;
              }
          ...
          if (!currentAbortController?.signal.aborted && lastRequestId === requestId) {
            console.log(`✅ [WS-AUTOCOMPLETE] Autocomplétion terminée [${requestId}]`);
            currentAbortController = null;
            lastRequestId = null;
          }
          ...
          if (error instanceof Error && (
            ...
            console.log(`🚫 [WS-AUTOCOMPLETE] Requête [${requestId}] annulée avec succès`);
            ...
            console.error(`❌ [WS-AUTOCOMPLETE] Erreur requête [${requestId}]:`, error);
  ...
  ws.on('close', () => {
    console.log(`🔌 [WS-AUTOCOMPLETE] Connexion fermée pour utilisateur ${user.id}`);
  ...
  ws.on('error', (error) => {
    console.error(`❌ [WS-AUTOCOMPLETE] Erreur WebSocket pour utilisateur ${user.id}:`, error);
    // Annuler toute requête en cours
    if (currentAbortController) {
      currentAbortController.abort();
    }
