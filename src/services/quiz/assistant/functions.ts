 ...
 */
export async function executeFunctionCall(toolCall: ToolCall): Promise<any> {
  ...
  try {
    ...
    throw new Error(`Arguments JSON invalides pour ${functionName}: ${toolCall.function.arguments}`);
  }
 ...
 */
async function generateGraphic(args: any): Promise<any> {
  console.log('📊 Génération graphique avec:', args);
  
  // Valider les paramètres requis
  const { config, type, library, description, dataValues, htmlContainer } = args;
  ...
  if (!config || Object.keys(config).length === 0) {
    ...
    finalConfig = await generateDefaultConfig(library, type, description, dataValues);
  }
...
async function generateDefaultConfig(library: string, type: string, description: string, dataValues: number[]): Promise<any> {
  ...
  try {
    ...
    if (aiGraphicResult && aiGraphicResult.config) {
      ...
      const correctedConfig = validateAndCorrectGraphicConfig(aiGraphicResult.config, description);
      console.log('🔍 Configuration après validation locale:', correctedConfig);
      
      // 🤖 VALIDATION IA AVANCÉE - Vérification scientifique par IA
      const aiValidatedConfig = await validateWithAI(correctedConfig, description);
      console.log('🧠 Configuration après validation IA:', aiValidatedConfig);
      
      return aiValidatedConfig;
    }
  } catch (error) {
    console.log('⚠️ Erreur génération IA, fallback vers configuration prédéfinie:', error);
  }
  
  // Fallback vers la logique prédéfinie si l'IA échoue
  console.log('🔧 Utilisation configuration prédéfinie comme fallback');
  const fallbackConfig = generateFallbackConfig(library, type, description, dataValues);
  
  // Appliquer la validation même sur le fallback
  const validatedFallback = validateAndCorrectGraphicConfig(fallbackConfig, description);
...
function validateAndCorrectGraphicConfig(config: any, description: string): any {
  const desc = description.toLowerCase();
  
  // Créer une copie profonde pour éviter les mutations
  const correctedConfig = JSON.parse(JSON.stringify(config));
  
  console.log('🔍 Validation scientifique pour:', desc.substring(0, 50) + '...');
  
  // ✅ CORRECTION 1: Fonctions mathématiques exactes
  if (desc.includes('quadratique') || desc.includes('parabole') || desc.includes('x²') || desc.includes('x^2')) {
  ...
  if (desc.includes('newton') || desc.includes('f=ma') || desc.includes('linéaire') || desc.includes('proportionnel')) {
  ...
  if (desc.includes('tangente') || desc.includes('dérivée')) {
    console.log('📐 Détection tangente → Application curve: smooth pour fonction + straight pour tangente');
    
    // S'assurer que le titre contient l'information sur la tangente
    if (correctedConfig.title && correctedConfig.title.text) {
      const currentTitle = correctedConfig.title.text;
      if (!currentTitle.includes('tangente') && !currentTitle.includes('dérivée')) {
    ...
    if (correctedConfig.series && correctedConfig.series.length > 1) {
      correctedConfig.series.forEach((serie: any, index: number) => {
        if (serie.name && serie.name.toLowerCase().includes('tangente')) {
          // Tangente = ligne droite rouge
          if (!correctedConfig.stroke || Array.isArray(correctedConfig.stroke)) {
  ...
  if (desc.includes('oscillation') || desc.includes('sinusoïd') || desc.includes('sin(')) {
    console.log('🌊 Détection oscillation → Conservation curve: smooth pour sin()');
  ...
  if (desc.includes('quadratique') || desc.includes('parabole')) {
    if (correctedConfig.series && correctedConfig.series[0] && correctedConfig.series[0].data) {
      ...
      for (let i = 0; i < data.length; i++) {
        const [x, y] = data[i];
        if (Math.abs(y - x * x) > 0.01) { // Tolérance de 0.01
          isSymmetric = false;
          console.log(`⚠️ Point non conforme à y=x²: (${x}, ${y}) devrait être (${x}, ${x*x})`);
        }
      }
...
function extractSubjectFromDescription(description: string): string {
  const subjects = {
    'physique': ['oscillation', 'force', 'énergie', 'onde', 'électrique', 'magnétique'],
    'mathématiques': ['fonction', 'quadratique', 'parabole', 'dérivée', 'intégrale', 'géométrie'],
...
function generateFallbackConfig(library: string, type: string, description: string, dataValues: number[]): any {
  if (library === 'apexcharts') {
    const desc = description.toLowerCase();
    
    // ✅ F=ma - Relation linéaire PARFAITE
    if (desc.includes('newton') || desc.includes('f=ma') || (desc.includes('force') && desc.includes('accélération'))) {
    ...
    if (desc.includes('quadratique') || desc.includes('parabole') || desc.includes('x²')) {
      const data = [];
    ...
    if (desc.includes('linéaire') || desc.includes('proportionnel') || desc.includes('droite')) {
...
async function generateAIGraphicConfig(params: {
  subject: string;
  topic: string;
  level: string;
  library: string;
 ...
 */
async function generateQuestionsArray(args: any): Promise<any> {
  console.log('❓ Génération questions avec:', args);
  
  const { questions } = args;
  
  if (!questions || !Array.isArray(questions)) {
    throw new Error('Array de questions manquant pour generate_questions_array');
  }
 ...
 */
async function generateSubjectWithDocuments(args: any): Promise<any> {
  console.log('📚 Génération sujet avec documents (API Wikipedia directe):', args);
  ...
  if (!title || !description || !documentTopics || !questionDistribution) {
    throw new Error('Paramètres manquants pour generate_subject_with_documents');
  }
  
  let searchResults: any[] = [];
  
  if (useFileUpload && uploadedFileIds && uploadedFileIds.length > 0) {
    console.log(`📤 Mode File Upload: utilisation de ${uploadedFileIds.length} fichiers uploadés`);
    ...
    try {
      ...
      console.log(`🤖 Recherche IA pour "${subjectName}" niveau ${targetLevel}`);
      console.log(`📝 Topics: ${documentTopics.join(', ')}`);
      
      const bestDocument = await searchWikipediaWithAI(subjectName, targetLevel, documentTopics, title);
      ...
      console.log(`✅ Document IA final créé: ${finalDocument.title} (${finalDocument.optimizedLength} chars)`);
  ...
  return {
    ...
    next_step: "OBLIGATOIRE: Tu DOIS maintenant appeler generate_questions_array pour créer des questions EXCLUSIVEMENT basées sur le contenu du document Wikipedia ci-dessus. NE PAS utiliser de connaissances générales, SEULEMENT le contenu du document fourni.",
 ...
 */
async function searchWikipediaAPI(query: string, limit: number = 5): Promise<any[]> {
  try {
 ...
 */
async function getWikipediaArticleContent(pageId: number, charLimit: number = 30000): Promise<any> {
  try {
    const fetch = (await import('node-fetch')).default;
 ...
 */
function calculateRelevanceScore(article: any, targetLevel: string): number {
  let score = 0;
  ...
  const educationalKeywords = {
    'BREVET': ['histoire', 'géographie', 'français', 'mathématiques', 'sciences', 'cours', 'leçon'],
    'BAC': ['analyse', 'étude', 'recherche', 'théorie', 'méthode', 'concept', 'développement'],
    'PARTIELS': ['université', 'recherche', 'académique', 'théorie', 'critique', 'analyse approfondie']
  };
  
  const keywords = educationalKeywords[targetLevel as keyof typeof educationalKeywords] || [];
  const keywordMatches = keywords.filter(keyword => snippet.includes(keyword)).length;
...
Réponds: {numéro}|{justification courte et précise}` 
    ...
    if (isNaN(bestIndex) || bestIndex < 0 || bestIndex >= searchResults.length) {
    ...
    const fullContent = await getWikipediaArticleContent(bestArticle.pageid, 100000); // Page complète
    
    if (!fullContent?.extract || fullContent.extract.length < 1000) {
      console.log(`⚠️ Article "${bestArticle.title}" trop court (${fullContent?.extract?.length || 0} chars), recherche d'un autre...`);
      ...
      if (searchResults.length > 1) {
        ...
        const secondContent = await getWikipediaArticleContent(secondBest.pageid, 100000);
        
        if (secondContent?.extract && secondContent.extract.length >= 1000) {
          console.log(`✅ 2ème article utilisé: "${secondBest.title}" (${secondContent.extract.length} chars)`);
          // Remplacer les variables pour continuer
          bestArticle.title = secondBest.title;
          bestArticle.pageid = secondBest.pageid;
          fullContent.extract = secondContent.extract;
        } else {
          throw new Error(`Tous les articles Wikipedia trouvés sont trop courts`);
        }
      } else {
        throw new Error('Contenu Wikipedia trop court ou inexistant et aucun fallback disponible');
      }
    }
    
    console.log(`📄 Contenu récupéré: ${fullContent.extract.length} caractères`);
...
Extrais les 6500 meilleurs caractères pour un quiz ${subject} niveau ${level}:` 
    ...
    if (!extractedContent || extractedContent.length < 2000) {
      console.log(`⚠️ Extraction IA courte (${extractedContent?.length || 0} chars), utilisation du contenu original complet`);
      // Fallback : utiliser tout le contenu disponible si l'extraction IA échoue
      return {
        title: bestArticle.title,
        content: fullContent.extract, // Utilise le contenu complet tel quel
        source: `Wikipedia - ${bestArticle.title}`,
        selectedBy: `IA-Smart-Selection (fallback: contenu original)`,
        originalLength: fullContent.extract.length,
...
async function correctQuizStandard(args: any): Promise<any> {
  ...
  try {
    ...
    console.log(`📝 Correction standard avec ${corrections.length} corrections`);
...
async function correctQuizWithGraphics(args: any): Promise<any> {
  throw new Error('correctQuizWithGraphics: Non implémenté dans le nouveau système');
}

async function correctQuizWithDocuments(args: any): Promise<any> {
  throw new Error('correctQuizWithDocuments: Non implémenté dans le nouveau système');
}

async function correctQuizComplete(args: any): Promise<any> {
  throw new Error('correctQuizComplete: Non implémenté dans le nouveau système');
}

async function validateWithAI(config: any, description?: any): Promise<any> {
