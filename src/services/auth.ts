...
if (!process.env.CLERK_SECRET_KEY) {
  console.error('❌ CRITIQUE: CLERK_SECRET_KEY manquante. Le service ne peut pas démarrer.');
  process.exit(1);
}
...
export class AuthService {
  ...
  static async verifyAuth(): Promise<AuthUser | null> {
    try {
      ...
      console.warn('verifyAuth() sans token - utiliser verifyToken() à la place');
      return null;
    } catch (error) {
  ...
  static async verifyToken(token: string): Promise<AuthUser | null> {
    try {
      ...
      if (!sessionToken || (sessionToken.exp && sessionToken.exp * 1000 < Date.now())) {
        console.warn('[AuthService] Tentative d\'utilisation d\'un token expiré.');
        return null;
      }
      ...
      if ((error as Error).message.includes('expired')) {
        // Ne pas polluer les logs pour les erreurs d'expiration normales
      } else {
