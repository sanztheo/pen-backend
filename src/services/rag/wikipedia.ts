...
export class WikipediaRAGSystem {
  ...
  async processWikipediaArticles(
    ...
    console.log(`🔄 [DEDUP] Vérification de ${titles.length} articles Wikipedia`);
    ...
    for (const article of articles) {
      try {
        ...
        if (dedupResult?.exists && !dedupResult.shouldUpdate) {
          console.log(`♻️ [DEDUP] Réutilisation: "${article.title}" (${dedupResult.chunksCount} chunks existants)`);
        ...
        if (dedupResult?.exists && dedupResult.shouldUpdate) {
          console.log(`🧹 [DEDUP] Nettoyage et re-embedding: "${article.title}"`);
        ...
        if (dedupResult?.exists && dedupResult.shouldUpdate) {
          ...
          source = await prisma.rAGSource.update({
            ...
            data: {
              originalUrl: fullArticle.url || `https://fr.wikipedia.org/wiki/${encodeURIComponent(article.title)}`,
              metadata: {
          ...
          console.log(`🌍 [WIKIPEDIA-GLOBAL] Source globale créée: "${article.title}" (ID: ${source.id})`);
        }
    ...
    for (const pageid of pageIds) {
      try {
        ...
        enrichedArticles.push({
          ...
          url: fullArticle.url || `https://fr.wikipedia.org/wiki/${encodeURIComponent(fullArticle.title)}`,
  ...
  private async getWikipediaFullContent(pageid: number): Promise<WikipediaArticle> {
    try {
      ...
      interface WikipediaQueryInfo {
        query?: { pages?: { [pageid: string]: { title: string; canonicalurl?: string; fullurl?: string; missing?: boolean; } } };
      }
      ...
      const sectionsData = await sectionsResponse.json() as WikipediaParseSections;
      
      // Traitement des sections
      const sections = await this.parseSections(sectionsData.parse?.wikitext?.['*'] || '');
      ...
      const extractData = await extractResponse.json() as WikipediaQueryExtract;
      const extract = extractData.query?.pages?.[pageid.toString()]?.extract || '';
  ...
  private async processWikipediaChunks(sourceId: string, chunks: RAGChunkInput[]): Promise<void> {
    ...
    const t0 = Date.now();
    console.log(`⚙️  [WIKIPEDIA] Embedding ${chunks.length} chunks (x${concurrency})…`);
    ...
    for (const batch of chunkArray(prepared, batchSize)) {
      ...
      console.log(`💾 [WIKIPEDIA] Inséré ${inserted}/${prepared.length} chunks…`);
    }

    console.log(`✅ [WIKIPEDIA] Terminé en ${Date.now() - t0} ms`);
  }

  private async calculateSectionRelevance(section: WikipediaSection, query: string): Promise<number> {
