...
export class ScoringService {
   ...
   */
  static async scoreToolResult(input: ScoreToolResultInput): Promise<ToolResultScore> {
    ...
    const heuristicScore = this.calculateHeuristicScore(toolName, result, query);

    // Si le score heuristique est très clair (>0.8 ou <0.3), pas besoin d'IA
    if (heuristicScore.overallScore > 0.8 || heuristicScore.overallScore < 0.3) {
      console.log(`📊 [SCORING] Score heuristique clair: ${heuristicScore.overallScore.toFixed(2)}`);
...
RETOURNE UNIQUEMENT UN JSON STRICT :
{
  ...
  "suggestions": ["<suggestion 1>", "<suggestion 2>"]
      ...
      const response = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'Tu es un évaluateur expert. Tu retournes UNIQUEMENT du JSON valide.'
          },
          {
            role: 'user',
      ...
      const score: ToolResultScore = {
        ...
        overallScore: (parsed.confidence || 0.5) * 0.4 + (parsed.relevance || 0.5) * 0.3 + (parsed.completeness || 0.5) * 0.3,
        reasoning: parsed.reasoning || 'Score calculé par IA',
        suggestions: parsed.suggestions || []
      };

      console.log(`📊 [SCORING] Score IA calculé: ${score.overallScore.toFixed(2)}`);
      return score;

    } catch (error) {
      console.warn(`⚠️ [SCORING] Erreur lors du scoring IA, fallback heuristique:`, error);
      return heuristicScore;
    }
  }
  ...
  private static calculateHeuristicScore(
    ...
    if (result.startsWith('❌') || result.includes('Erreur') || result.includes('Error')) {
      confidence = 0.1;
      relevance = 0.1;
      completeness = 0.1;
    ...
    // RÈGLE 2 : Détecter les résultats vides
      ...
      relevance = 0.3;
      completeness = 0.2;
      suggestions.push('Résultat vide ou insuffisant, explorer d\'autres sources');
    }
    // RÈGLE 3 : Résultats avec contenu
      ...
      const matchCount = queryWords.filter(word => result.toLowerCase().includes(word)).length;
    ...
    if (toolName === 'list_available_sources' || toolName === 'list_global_wikipedia_sources') {
      ...
      if (sourceCount > 0) {
        ...
        suggestions.push(`${sourceCount} sources trouvées, procéder à la sélection`);
      }
    }
    ...
    const overallScore = confidence * 0.4 + relevance * 0.3 + completeness * 0.3;
  ...
  static async adjustStrategy(
    executedTools: Array<{ name: string; score?: ToolResultScore; result: string }>,
    query: string,
    context: {
      useWeb: boolean;
    ...
    console.log(`🔄 [STRATEGY-ADJUST] Analyse de ${executedTools.length} tools exécutés...`);
    ...
    console.log(`📊 [STRATEGY-ADJUST] Score moyen: ${avgScore.toFixed(2)}`);
    ...
    if (context.hasSpecificSource && context.availableSourcesCount === 1) {
      ...
      if (lastScore > 0.7) {
        ...
        return {
          ...
          shouldUseWeb: context.useWeb && !executedTools.some(t => t.name === 'search_web'),
          shouldStop: !context.useWeb,
          confidence: 0.9,
          reasoning: 'La page sélectionnée contient des informations de qualité. ' +
                     (context.useWeb ? 'Web disponible pour enrichissement optionnel.' : 'Information suffisante.'),
        ...
        return {
          ...
          shouldStop: false,
          confidence: 0.7,
          reasoning: 'La page sélectionnée contient des informations partielles. Exploration recommandée.',
          suggestedTools: context.useWeb ? ['search_web'] : ['list_available_sources'],
          priority: 'medium'
        };
      } else {
        ...
        return {
          ...
          shouldStop: false,
          confidence: 0.9,
          reasoning: 'La page sélectionnée est insuffisante. Exploration d\'autres sources fortement recommandée.',
          suggestedTools: context.useWeb ? ['search_web', 'list_available_sources'] : ['list_available_sources'],
          priority: 'high'
        };
      }
    }
    ...
    if (context.hasSpecificSource && context.availableSourcesCount > 1) {
      if (avgScore > 0.7) {
        return {
          ...
          shouldStop: !context.useWeb,
          confidence: 0.85,
          reasoning: 'Les sources sélectionnées fournissent des informations complètes.',
          suggestedTools: context.useWeb ? ['search_web'] : [],
          priority: 'low'
        };
      } else {
        return {
          ...
          shouldStop: false,
          confidence: 0.8,
          reasoning: 'Les sources sélectionnées sont partielles. Exploration recommandée.',
          suggestedTools: context.useWeb ? ['search_web'] : ['list_available_sources'],
          priority: 'medium'
        };
      }
    ...
    if (!context.hasSpecificSource || context.availableSourcesCount === 0) {
      const hasListedSources = executedTools.some(t =>
        t.name === 'list_available_sources' || t.name === 'list_global_wikipedia_sources'
      );
      const hasReadSources = executedTools.some(t =>
      ...
      if (!hasListedSources) {
        return {
          ...
          shouldStop: false,
          confidence: 0.95,
          reasoning: 'Mode exploration : lister d\'abord les sources disponibles.',
          suggestedTools: ['list_available_sources', 'list_global_wikipedia_sources'],
          priority: 'critical'
        };
      }
      ...
      if (hasListedSources && !hasReadSources) {
        return {
          ...
          shouldStop: false,
          confidence: 0.9,
          reasoning: 'Sources listées : sélectionner et lire les plus pertinentes.',
      ...
      if (avgScore > 0.7) {
        return {
          ...
          confidence: 0.85,
          reasoning: 'Sources locales de qualité trouvées. ' +
                     (context.useWeb && !hasSearchedWeb ? 'Web disponible pour enrichissement.' : 'Information suffisante.'),
          suggestedTools: context.useWeb && !hasSearchedWeb ? ['search_web'] : [],
        ...
        return {
          ...
          shouldStop: false,
          confidence: 0.8,
          reasoning: 'Sources locales insuffisantes. ' +
                     (context.useWeb ? 'Recherche web recommandée.' : 'Explorer d\'autres sources.'),
          suggestedTools: context.useWeb ? ['search_web'] : ['search_rag_chunks'],
          priority: 'high'
        };
      }
    }
    ...
    if (context.useWeb && !executedTools.some(t => t.name === 'search_web')) {
      if (avgScore < 0.6) {
        return {
          ...
          shouldStop: false,
          confidence: 0.9,
          reasoning: 'Résultats actuels insuffisants. Recherche web fortement recommandée.',
        ...
        return {
          ...
          shouldStop: false,
          confidence: 0.7,
          reasoning: 'Résultats partiels. Recherche web recommandée pour enrichir.',
          suggestedTools: ['search_web'],
          priority: 'medium'
        };
      } else {
        return {
          ...
          shouldStop: false,
          confidence: 0.6,
          reasoning: 'Résultats de qualité. Recherche web optionnelle pour enrichissement.',
          suggestedTools: ['search_web'],
          priority: 'low'
        };
      }
