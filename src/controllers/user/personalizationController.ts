...
export const getPersonalization = async (req: Request, res: Response) => {
  try {
    if (!req.user) return res.status(401).json({ success: false, error: 'Utilisateur non authentifié' });
...
export const updatePersonalization = async (req: Request, res: Response) => {
  try {
    ...
    const currentPersona = (currentSettings.personalization as Personalization) || {};
    ...
    return res.json({ success: true, message: 'Personnalisation mise à jour', data: merged });
  } catch (error) {
