...
export class RAGCleanupService {
  ...
  async cleanupUnusedSources(options: CleanupOptions): Promise<CleanupStats> {
    ...
    console.log(`🧹 [CLEANUP] Démarrage nettoyage - Age max: ${maxAge} jours, DryRun: ${dryRun}`);
    ...
    console.log(`🧹 [CLEANUP] ${candidateSources.length} sources candidates au nettoyage`);
    ...
    for (let i = 0; i < candidateSources.length; i += batchSize) {
      ...
      console.log(`🧹 [CLEANUP] Lot ${Math.floor(i/batchSize) + 1}/${Math.ceil(candidateSources.length/batchSize)} traité: ${batchStats.sourcesDeleted} sources, ${batchStats.chunksDeleted} chunks`);
    }

    const duration = Date.now() - startTime;
    
    console.log(`🧹 [CLEANUP] Terminé en ${duration}ms - Sources: ${totalSourcesDeleted}, Chunks: ${totalChunksDeleted}, Espace: ${totalSpaceFreed.toFixed(2)}MB`);
   ...
   */
  private async processBatch(sources: any[], dryRun: boolean): Promise<CleanupStats> {
    let chunksDeleted = 0;
    let sourcesDeleted = 0;
    let spaceFreed = 0;

    for (const source of sources) {
      const chunkCount = (source as any)._count.chunks;
      ...
      console.log(`${dryRun ? '🔍 [DRY-RUN]' : '🗑️ [DELETE]'} Source: "${source.title}" (${chunkCount} chunks, ${estimatedSizeKB}KB)`);
    }
  ...
  async updateLastUsed(sourceId: string): Promise<void> {
    try {
      ...
      console.error(`🚨 [CLEANUP] Erreur mise à jour lastUsedAt pour ${sourceId}:`, error);
    }
  }
  ...
  async updateLastUsedBatch(sourceIds: string[]): Promise<void> {
    ...
    try {
      ...
      console.log(`📊 [CLEANUP] LastUsedAt mis à jour pour ${sourceIds.length} sources`);
    } catch (error) {
  ...
  async previewCleanup(maxAge: number = 7, includeUserSources: boolean = false) {
    ...
    return {
      ...
      totalChunks: candidates.reduce((sum, s) => sum + ((s as any)._count.chunks || 0), 0),
      estimatedSpaceMB: candidates.reduce((sum, s) => sum + (((s as any)._count.chunks || 0) / 1024), 0),
